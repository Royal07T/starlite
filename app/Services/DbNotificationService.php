...
class DbNotificationService
{
    ...
    public function getAssignedAssignmentNotification($content, $data)
    {
        ...
        if (Str::contains($emailTemplate['content'], '{assignedAssignmentUrl}')) {
            ...
            $emailTemplate['link_text']     = __('notification_template.view_assignment');
            $emailTemplate['content']       = Str::replace("{assignedAssignmentUrl}", '', $emailTemplate['content']);   
        }

        $emailTemplate['content'] = Str::replace('{assignmentTitle}', $data['assignmentTitle'], $emailTemplate['content']);
        $emailTemplate['content'] = Str::replace('{studentName}', $data['studentName'], $emailTemplate['content']);
        $emailTemplate['content'] = Str::replace('{tutorName}', $data['tutorName'], $emailTemplate['content']);
        $emailTemplate['title'] = Str::replace('{assignmentTitle}', $data['assignmentTitle'], $emailTemplate['title']);
    ...
    /**
     * Parse booking assignment notification template
     *
     * @param array $content
     * @param array $data
     * @return array
     */
    public function getBookingassignedNotification($content, $data)
    {
        $notificationTemplate = $content;

        if (isset($data['bookingUrl']) && !empty($data['bookingUrl'])) {
            $notificationTemplate['has_link'] = true;
            $notificationTemplate['link_target'] = $data['bookingUrl'];
            $notificationTemplate['link_text'] = 'View Booking';
        }

        // Replace variables in content
        $notificationTemplate['content'] = Str::replace('{tutorName}', $data['tutorName'] ?? '', $notificationTemplate['content']);
        $notificationTemplate['content'] = Str::replace('{studentName}', $data['studentName'] ?? '', $notificationTemplate['content']);
        $notificationTemplate['content'] = Str::replace('{subjectName}', $data['subjectName'] ?? '', $notificationTemplate['content']);
        $notificationTemplate['content'] = Str::replace('{bookingDate}', $data['bookingDate'] ?? '', $notificationTemplate['content']);
        $notificationTemplate['content'] = Str::replace('{bookingTime}', $data['bookingTime'] ?? '', $notificationTemplate['content']);
        $notificationTemplate['content'] = Str::replace('{bookingType}', $data['bookingType'] ?? 'assigned', $notificationTemplate['content']);
        
        // Replace variables in title
        $notificationTemplate['title'] = Str::replace('{bookingType}', $data['bookingType'] ?? 'assigned', $notificationTemplate['title']);
