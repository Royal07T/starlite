...
class BookingService
{
            ...
            if (Module::has('kupondeal') && Module::isEnabled('kupondeal')) {
                $query->with(
                    ...
                    fn ($query) => $query->select('id', 'couponable_id', 'couponable_type', 'code', 'discount_type', 'discount_value')
                        ->where('status', StatusCast::$statuses['active'])->where('expiry_date', '>=', now())
                        ->where('auto_apply', 1)
                        ->oldest()
                        ->limit(1)
                );
    ...
    public function getUserBookings($date, $showBy = 'daily', $filters = [])
    {
        ...
        $bookings = SlotBooking::select('id', 'tutor_id', 'student_id', 'user_subject_slot_id', 'session_fee', 'start_time', 'end_time', 'status')
            ...
            ->orWhere(function ($query) use ($date) {
                // Allow unassigned bookings for students
                if ($this->user->role == 'student') {
                    $query->whereNull('user_subject_slot_id')
                        ->whereStudentId($this->user->id)
                        ->where('start_time', '>=', $date['start_date'])
                        ->where('end_time', '<=', $date['end_date']);
                }
            })
            ->when($this->user->role == 'tutor', fn ($query) => $query->whereTutorId($this->user->id)->whereIn('status', [BookingStatus::$statuses['active']]))
            ->when($this->user->role == 'student', function ($query) {
                $query->whereStudentId($this->user->id);
                // Note: The orWhere above might conflict with the main query structure if not careful.
                // Re-structuring:
                // The original query mandated 'slot'.
                // We want: (HasSlot AND SlotMatches) OR (NoSlot AND IsMyUnassigned)
            })
            ->get();
    ...
    public function addUserSubjectGroupSessions($slots = [])
    {
    ...
    public function enrollFreeCourse($resourceId)
    {
        ...
        try {
            ...
            (new CourseService)->addStudentCourse($courseData);
        ...
        try {
            ...
            if (! empty($bundle->courses)) {
                foreach ($bundle->courses as $course) {
                    $alreadyHaveCourse = (new CourseService)->getStudentCourse(
                        courseId: $course->id,
