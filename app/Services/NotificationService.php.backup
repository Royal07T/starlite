...
class NotificationService
{
    ...
    /**
     * Parse booking assignment email template
     *
     * @param array $content
     * @param array $data
     * @return array
     */
    public function getBookingAssignedEmail($content, $data)
    {
        $emailTemplate = array();
        foreach ($content as $key => &$value) {
            $content[$key] = Str::replace('{tutorName}', $data['tutorName'] ?? '', $value);
            $content[$key] = Str::replace('{studentName}', $data['studentName'] ?? '', $value);
            $content[$key] = Str::replace('{subjectName}', $data['subjectName'] ?? '', $value);
            $content[$key] = Str::replace('{bookingDate}', $data['bookingDate'] ?? '', $value);
            $content[$key] = Str::replace('{bookingTime}', $data['bookingTime'] ?? '', $value);
            $content[$key] = Str::replace('{bookingType}', $data['bookingType'] ?? 'assigned', $value);
            $content[$key] = Str::replace('{bookingUrl}', $data['bookingUrl'] ?? '#', $value);
        }
        $emailTemplate = $content;
        
        // If there's a button in the content, replace it with proper HTML
        if (Str::contains($emailTemplate['content'], '{bookingUrl}')) {
            $btnHtml = view('components.email.button', [
                'btnText' => trans('View Booking Details'), 
                'btnUrl' => $data['bookingUrl'] ?? '#'
            ])->render();
            $emailTemplate['content'] = Str::replace(
