...
class CourseService
{
    ...
    {
        $enrollment = Enrollment::where('student_id', $courseData['student_id'])
            ->where('course_id', $courseData['course_id'])
            ->first();

        if ($enrollment) {
            // If it's a session pack purchase, increment sessions
            if (isset($courseData['total_sessions']) && $courseData['total_sessions'] > 0) {
                $enrollment->increment('total_sessions', $courseData['total_sessions']);
            }
            return true;
        }

        return Enrollment::create($courseData);
    }
