...
| Database | MySQL (`.env`: `starlite_db`) |
...
| Browsershot/Chrome | `CHROME_PATH=/usr/bin/google-chrome` | for screenshots/PDFs |
...
|---|---|---|---|
| S1 | **`.env` committed with live secrets** | `MAIL_PASSWORD='TheGhost5748!'`, DB config, in git; `ai_assistant/.env` also tracked | `git rm --cached .env ai_assistant/.env`, rotate creds, add `.env.example` |
